...
import { ContactType, CONTACT_TYPE_LABELS } from '@/types/contacts';
...
export default function EditContactPage() {
  ...
  const { contacts, updateContact, getContactOptions } = useSupabaseContacts();
  useScrollToTop();
  ...
  useEffect(() => {
    if (contact) {
      ...
      setCustomType(contact.customType || '');
      ...
      setParentContactId(contact.parentContactId || '');
      setUseWhatsappSameAsPhone(contact.phone === contact.whatsapp);
    }
  }, [contact]);

  const contactOptions = getContactOptions().filter(c => c.id !== id);
  ...
  if (!contact) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <p className="text-muted-foreground text-sm">جهة الاتصال غير موجودة</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background pb-6">
      ...
      <header className="sticky top-0 z-50 bg-gradient-primary text-primary-foreground shadow-lg">
        <div className="container flex h-12 items-center gap-3">
          ...
          <h1 className="text-sm font-bold">تعديل جهة الاتصال</h1>
        </div>
      </header>
