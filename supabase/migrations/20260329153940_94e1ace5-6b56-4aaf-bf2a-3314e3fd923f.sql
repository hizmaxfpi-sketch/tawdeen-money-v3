...
BEGIN
  ...
  IF v_existing_company_id IS NULL THEN
    v_company_name := COALESCE(NULLIF(TRIM(NEW.raw_user_meta_data->>'company_name'), ''), 'شركة جديدة');
    ...
    IF NOT EXISTS (SELECT 1 FROM public.user_roles WHERE is_active = true) THEN
      ...
      VALUES (NEW.id, v_company_name, 'active')
      RETURNING id INTO v_company_id;
    ELSE
      INSERT INTO public.companies (owner_user_id, name, status)
      VALUES (NEW.id, v_company_name, 'pending')
      RETURNING id INTO v_company_id;
    END IF;
