...
-- 1. إضافة عمود الإقفال اليدوي للحاويات
ALTER TABLE public.containers ADD COLUMN IF NOT EXISTS is_manually_closed BOOLEAN NOT NULL DEFAULT false;

-- 2. تحديث create_shipment_with_accounting
CREATE OR REPLACE FUNCTION public.create_shipment_with_accounting(
  p_user_id uuid, p_container_id uuid, p_client_id uuid DEFAULT NULL,
  p_client_name text DEFAULT '', p_client_code text DEFAULT NULL,
  p_recipient_name text DEFAULT NULL, p_goods_type text DEFAULT '',
  p_length numeric DEFAULT 0, p_width numeric DEFAULT 0, p_height numeric DEFAULT 0,
  p_quantity integer DEFAULT 1, p_weight numeric DEFAULT 0, p_cbm numeric DEFAULT NULL,
  p_price_per_meter numeric DEFAULT 0, p_amount_paid numeric DEFAULT 0,
  p_tracking_number text DEFAULT NULL, p_notes text DEFAULT NULL,
  p_china_expenses numeric DEFAULT 0, p_sea_freight numeric DEFAULT 0,
  p_port_delivery_fees numeric DEFAULT 0, p_customs_fees numeric DEFAULT 0,
...
-- 3. تحديث update_shipment_with_accounting
CREATE OR REPLACE FUNCTION public.update_shipment_with_accounting(
  ...
  p_length numeric DEFAULT NULL, p_width numeric DEFAULT NULL, p_height numeric DEFAULT NULL,
  p_quantity integer DEFAULT NULL, p_price_per_meter numeric DEFAULT NULL,
  p_amount_paid numeric DEFAULT NULL, p_tracking_number text DEFAULT NULL, p_notes text DEFAULT NULL
)
...
BEGIN
  ...
  IF v_old.client_id IS NOT NULL AND v_price_changed THEN
    ...
    -- حذف أقدم عملية مالية مرتبطة بالشحنة (باستخدام subquery بدل LIMIT)
...
-- 4. تحديث delete_shipment_with_accounting لحذف العمليات المالية
