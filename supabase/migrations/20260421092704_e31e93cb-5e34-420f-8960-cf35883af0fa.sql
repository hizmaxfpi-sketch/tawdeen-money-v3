...
BEGIN
  ...
  IF v_item_count = 1 THEN
    SELECT di.account_id INTO v_account_id
    FROM public.obligation_draft_items di
    JOIN public.ledger_accounts la ON la.id = di.account_id
    WHERE di.draft_id = p_draft_id
    LIMIT 1;
  ELSE
    SELECT COUNT(DISTINCT di.account_id), COUNT(di.account_id)
